...
const navlinks = [
  ...
  {
    label: 'Sample',
    href: '/result/ca0b6e1b-0796-44a2-baaa-0fab874b3631',
    routeIdentifier: '/result',
  },
];
...
const NavLink = ({ label, href, routeIdentifier }) => {
  ...
  const inactiveLinkStyle =
    'block py-2 pl-3 pr-4 text-gray-700 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 md:dark:hover:text-white dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700 cursor-pointer';
  ...
  return (
    <div
      ...
      className={
        location.pathname === routeIdentifier ||
...
const Navbar = () => {
  ...
  return (
    <nav
      id='navbar'
      className='bg-transparent backdrop-blur px-2 sm:px-4 py-2.5 fixed w-full z-20 top-0 left-0 border-b border-gray-400 dark:border-gray-600'
    >
      <div
        id='container'
      ...
      >
        <a id='logo' href='/' className='flex items-center'>
          ...
          <span
            id='title'
            className='self-center text-xl font-bold whitespace-nowrap dark:text-teal-500'
          >
            VidAssembly
          </span>
        </a>
            ...
            {navlinks.map((navlink) => (
              <li key={navlink.label} onClick={() => toggleDrawer(true)}>
                <NavLink
                  label={navlink.label}
                  href={navlink.href}
                  routeIdentifier={navlink.routeIdentifier}
                />
              </li>
            ))}
          </ul>
